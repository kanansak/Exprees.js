...
router.use(express.static(path.join(__dirname, 'uploads-icon')));

// อื่นๆ ที่คุณมีอาจจะต้องกำหนดแบบเดียวกัน

router.get('/uploads-icon/:deviceId', (req, res) => {
  const deviceId = req.params.deviceId;
  console.log(deviceId);
  const uploadsFileName = deviceId + '.png'; // สร้างชื่อไฟล์ด้วย deviceId
  const filePath = path.join(__dirname, 'uploads-icon', uploadsFileName);
